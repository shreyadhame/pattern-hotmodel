...
if __name__ == "__main__":
    ...
    x = dfl['MIROC6']
    y = dfl['CanESM5']
    sns.histplot(x,stat='count', kde=True, fill=True,linewidth=0.,alpha=0.3,color='#E66100',ax=ax1,kde_kws={'cut':2})
    # sns.kdeplot(x,color='#E66100',ax=ax1,linewidth=2)
    sns.histplot(y,stat='count', kde=True, fill=True, linewidth=0.,alpha=0.3,color='#5D3A9B',ax=ax1,kde_kws={'cut':2})
    ...
    ax2.text(-1.2,2.5,'R$^2$ = '+str(np.round(ew_r[ind],2)))
    ax2.text(-1.3,6.2,'1970 - 1995',fontsize=20)
    ...
    x = dfr['MIROC6']
    y = dfr['CanESM5']
    sns.histplot(x,stat='count', kde=True, fill=True,linewidth=0.,alpha=0.3,color='#E66100',ax=ax4,kde_kws={'cut':2})
    # sns.kdeplot(x,color='#E66100',ax=ax4,linewidth=2)
    sns.histplot(y,stat='count', kde=True, fill=True, linewidth=0.,alpha=0.3,color='#5D3A9B',ax=ax4,kde_kws={'cut':2})
    ...
    ax4.legend(lines, title,loc='upper right',labelspacing=0.9,frameon=False)
    ...
    # Add text for R2 and slope and period
    ax3.text(1.4,2.5,'R$^2$ = '+str(np.round(ew_r[ind],2)))
    # ax3.text(3.6,6.2,'1990 - 2015',fontsize=20)
    ax3.text(1.4,6.2,'1990 - 2015',fontsize=20)

    ax4.set_title('(d)',loc='center')

    fig.set_facecolor("white") # set figure background color to white
    plt.savefig('Fig2_iqr.png', dpi=300, facecolor='white', bbox_inches="tight")
